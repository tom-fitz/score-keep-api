...
//	id := c.Param("id")
//	if id == "" {
//		c.JSON(statusBadRequest, gin.H{"error": "league id required"})
//		return
//	}
//
//	if err := validateLeague(id, h.db); err != nil {
//		c.JSON(statusBadRequest, gin.H{"error": fmt.Sprintf("error validating league: %v", err)})
//		return
//	}
//
//	teams, err := parseCSVFile(c, "teams")
//	if err != nil {
//		c.JSON(statusBadRequest, gin.H{"error": err.Error()})
//		return
//	}
//
//	players, err := parseCSVFile(c, "players")
//	if err != nil {
//		c.JSON(statusBadRequest, gin.H{"error": err.Error()})
//		return
//	}
//
//	if err := InsertTeamData(h.db, teams); err != nil {
//		c.JSON(statusInternalServerError, gin.H{"error": fmt.Sprintf("InsertTeamData: %v", err)})
//		return
//	}
//	if err := InsertPlayerData(h.db, players); err != nil {
//		c.JSON(statusInternalServerError, gin.H{"error": fmt.Sprintf("InsertPlayerData: %v", err)})
//		return
//	}
//
//	c.JSON(statusOK, gin.H{"message": "Import successful"})
//}
//
//func parseCSVFile(c *gin.Context, fieldName string) ([]map[string]string, error) {
//	file, _, err := c.Request.FormFile(fieldName)
//	if err != nil {
//		return nil, fmt.Errorf("%s.csv file not provided", fieldName)
//	}
//	defer file.Close()
//
//	reader := csv.NewReader(file)
...
//		}
//		if err != nil {
//			return nil, fmt.Errorf("error reading %s.csv: %v", fieldName, err)
//		}
//
//		if firstRow {
//			firstRow = false
...
//			row[fmt.Sprintf("col%d", i+1)] = value
//		}
